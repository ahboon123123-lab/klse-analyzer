...
$(document).ready(function() {
	...
	//动态添加大转盘的奖品与奖品区域背景颜色
	turnplate.restaraunts = [languages.turnplateText5, languages.turnplateText7, languages.turnplateText6, languages.turnplateText5, languages.turnplateText7, languages.turnplateText6];
	turnplate.colors = ["#b6a34a", "#caa291","#B2867E"];
...
//选股的点击事件
	$('.pointer').click(function() {
		...
		if(device == 2) {//与IOS的交互
			...
			path = "getxgResult_new.shop?"+encodeURI(cs)+"&type="+selectType+"&day="+selectDay;
		} else if (device == 1) {//与Android的交互
			 cs = window.wxc.getPlateRoateState();
			path = "getxgResult_new.shop?"+encodeURI(cs)+"&type="+selectType+"&day="+selectDay;
		} else {
			cs = "jwcode=7c4f52d2f8137c637d750d6e273367a69886ecd782e17d84&name=NDXS&type=2&zbid=78&yzcode=129e7e866fc1b8d6a6b86deb8a623f7b76c8a0fd241050127ccfe675643ab243&day=7";
...
function globalI18n() {
	...
	document.getElementById("zhizhen3").src = languages.zhizhen3;
	document.getElementById("bigImg3").src = languages.bigImg3;
	$('#dayType1').html(languages.day1);
	$('#dayType2').html(languages.day2);
	$('#dayType3').html(languages.day3);
	$('#jnsf_desc_title').html(languages.jnsf_desc_title);
	$('#jnsf_desc').html(languages.jnsf_desc);
}
	...
	if(s == "1") {
		...
		for(var i = 0; i < aCat.length; i++) {
			aCat[i].onchange = function() {
				...
				var danxuan = document.getElementById("danquan");
				if(selectType == "1"){
					danxuan.style.visibility = "hidden";
				}else{
					danxuan.style.visibility = "visible";
				}
			}
		}
		
		//控制单选按钮==天数
		var dCat = document.getElementsByName('d_sel');
		for(var i = 0; i < dCat.length; i++) {
			dCat[i].onchange = function() {
...
function drawRouletteWheel() {
	...
	if(canvas.getContext) {
		...
		for(var i = 0; i < turnplate.restaraunts.length; i++) {
			...
			var insideRadius = dwidth / 2 - dwidth * 0.352;
...
function gethczpwords(ctx,text,line_height){
	...
	}else if(text.indexOf("Bottom") > 0) { //流量包
		var texts = text.split("Bottom");
		for(var j = 0; j < texts.length; j++) {
			ctx.font = j == 0 ? 'bold 14px Microsoft YaHei' : 'bold 14px Microsoft YaHei';//'14px Microsoft YaHei';
			if(j == 0) {
				ctx.fillText(texts[j], -ctx.measureText(texts[j]).width / 2, j * line_height);
			} else {
				ctx.fillText("Bottom", -ctx.measureText("Bottom").width / 2, j * line_height);
			}
		}
	}else {
