...
$(document).ready(function() {
	...
	//动态添加大转盘的奖品与奖品区域背景颜色
	turnplate.restaraunts = [languages.turnplateText3, languages.turnplateText7, languages.turnplateText4, languages.turnplateText3, languages.turnplateText7, languages.turnplateText4];
	turnplate.colors = ["#df61b3","#f6c508",  "#95CE8B"];
...
//选股的点击事件
	...
	$('.pointer').click(function() {
		...
		if(device == 2) {//与IOS的交互
			...
			cs = "jwcode=7c4f52d2f8137c637d750d6e273367a69886ecd782e17d84&name=大牛有形&type=2&zbid=77&yzcode=129e7e866fc1b8d6a6b86deb8a623f7b76c8a0fd241050127ccfe675643ab243";
...
function globalI18n() {
	...
	$('#dnyx_desc_title').html(languages.dnyx_desc_title);
	$('#dnyx_desc').html(languages.dnyx_desc);
	document.getElementById("zhizhen2").src = languages.zhizhen2;
	document.getElementById("bigImg2").src = languages.bigImg2;
}
...
function drawRouletteWheel() {
	...
	if(canvas.getContext) {
		...
		for(var i = 0; i < turnplate.restaraunts.length; i++) {
			...
			var outsideRadius = dwidth / 2 - dwidth * 0.035;
			var insideRadius = dwidth / 2 - dwidth * 0.336;
...
function gethczpwords(ctx,text,line_height){
	...
	}else if(text.indexOf("Rhythm") > 0) { //流量包
		var texts = text.split("Rhythm");
		for(var j = 0; j < texts.length; j++) {
			ctx.font = j == 0 ? 'bold 14px Microsoft YaHei' : 'bold 14px Microsoft YaHei';//'14px Microsoft YaHei';
			if(j == 0) {
				ctx.fillText(texts[j], -ctx.measureText(texts[j]).width / 2, j * line_height);
			} else {
				ctx.fillText("Rhythm", -ctx.measureText("Rhythm").width / 2, j * line_height);
			}
		}
	}else if(text.indexOf("Chips") > 0) { //流量包
		var texts = text.split("Chips");
		for(var j = 0; j < texts.length; j++) {
			ctx.font = j == 0 ? 'bold 14px Microsoft YaHei' : 'bold 14px Microsoft YaHei';//'14px Microsoft YaHei';
			if(j == 0) {
				ctx.fillText(texts[j], -ctx.measureText(texts[j]).width / 2, j * line_height);
			} else {
				ctx.fillText("Chips", -ctx.measureText("Chips").width / 2, j * line_height);
			}
		}
	}else {
